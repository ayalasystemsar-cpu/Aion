...
# C. ROL: SUPERVISOR
    ...
    if not st.session_state.sup_autenticado:
        ...
        with col_btn2:
            if st.button("REFRESCAR SISTEMA", key="btn_refrescar_sistema"): st.rerun()

        t_vis_qr, t_car_tac, t_com_sup = st.tabs(["Visita QR", "Carga Táctica", "Comunicación"])
        
        with t_vis_qr:
            opciones_servicios = df_objetivos_filtrados['OBJETIVO'].unique() if not df_objetivos_filtrados.empty else []
            if len(opciones_servicios) == 0: opciones_servicios = ["SIN OBJETIVOS ASIGNADOS"]
            ...
            if not df_mapa_sup.empty:
                ...
                for _, r in df_mapa_sup.iterrows():
                    ...
                    folium.Marker([r['LATITUD'], r['LONGITUD']], tooltip=folium.Tooltip(tooltip_html, sticky=True), icon=folium.Icon(color="blue", icon="shield", prefix="fa")).add_to(m_visor)
            else:
                m_visor = folium.Map(location=[-34.6037, -58.3816], zoom_start=12, tiles="CartoDB dark_matter")
                st.warning("⚠️ No se detectaron coordenadas válidas en la matriz.")
...
# D. ROL: GERENCIA
    ...
    st.markdown('<h2 style="color:#00E5FF; font-family:\'Orbitron\', sans-serif; font-size:24px;">Comando Estratégico: DIRECCIÓN GENERAL</h2>', unsafe_allow_html=True)
    m1, m2, m3, m4 = st.columns(4)
    m1.metric("Ahorro de Riesgo (Estimado)", "$ 1.200.000")
    m2.metric("Nivel de Cobertika", "47/93")
    ...
    with t_ejecucion_ger:
        ...
        with col_g1:
            ...
            if st.button("Solicitar Alta a Admin", key="btn_ger_alta"):
                if g_alta_nom.strip(): escribir_registro_nube("PETICIONES", [obtener_hora_argentina(), st.session_state.user_sel, "ALTA", "OBJETIVO", f"{g_alta_nom} | ASIG: {g_alta_asig}"])
            st.markdown('</div>', unsafe_allow_html=True)
        with col_g2:
            st.markdown('<div class="panel-novedad">', unsafe_allow_html=True)
            ...
            if st.button("Solicitar Baja a Admin", key="btn_ger_baja"): escribir_registro_nube("PETICIONES", [obtener_hora_argentina(), st.session_state.user_sel, "BAJA", "OBJETIVO", g_baja_obj])
            st.markdown('</div>', unsafe_allow_html=True)
    ...
    with t_tab_auditoria:
        ...
        if not df_ger_maps.empty:
            for _, r in df_ger_maps.iterrows(): folium.Marker([r['LATITUD'], r['LONGITUD']], tooltip=f"OBJETIVO: {r['OBJETIVO']} | SUPERVISOR: {r.get('SUPERVISOR', 'N/A')}", icon=folium.Icon(color="blue", icon="shield", prefix="fa")).add_to(m_visor)
