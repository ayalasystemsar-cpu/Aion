...
if st.session_state.rol_sel == "JEFE DE OPERACIONES":
    ...
    tab1, tab2, tab3 = st.tabs(["Centro de Crisis", "Ejecución", "Auditoría"])
    ...
    with tab1:
        ...
        for _, r in df_objetivos.dropna(subset=['LATITUD', 'LONGITUD']).iterrows():
            folium.Marker([r['LATITUD'], r['LONGITUD']], popup=r['OBJETIVO'], tooltip=f"Auditar: {r['OBJETIVO']}", icon=folium.Icon(color="blue", icon="shield")).add_to(m)
        ...
        if obj_click:
            ...
            with c2:
                st.markdown('<div class="panel-novedad">', unsafe_allow_html=True)
                st.markdown("**🔄 NOVEDADES RECIENTES:**", unsafe_allow_html=True)
