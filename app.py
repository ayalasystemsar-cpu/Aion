...
# C. ROL: SUPERVISOR (FILTRADO PERIMETRAL EXACTO Y MAPAS CON CENTRO DINÁMICO)
    ...
    if not st.session_state.sup_autenticado:
        st.info("🔒 Estación Bloqueada.")
    else:
        # 1. Normalización estricta
        sup_norm = str(st.session_state.user_sel).strip().upper()
        
        # 2. FILTRADO EXACTO AQUÍ
        # Este df_f solo contendrá filas donde la columna SUPERVISOR sea idéntica al usuario logueado
        df_f = df_objetivos[df_objetivos['SUPERVISOR'].astype(str).str.strip().str.upper() == sup_norm]
        
        st.markdown(f'<div class="estacion-titulo">📱 ESTACIÓN: {sup_norm}</div>', unsafe_allow_html=True)
        
        # DEBUG: Si no ves puntos, esto te dirá cuántos encontró
        st.caption(f"Objetivos filtrados encontrados: {len(df_f)}")
        
        if not df_f.empty:
            # Mapa centrado automáticamente en los objetivos filtrados
            m = folium.Map(location=[df_f['LATITUD'].mean(), df_f['LONGITUD'].mean()], zoom_start=14, tiles="CartoDB dark_matter")
            for _, r in df_f.iterrows():
                # Tooltip con nombre de objetivo y responsable
                tooltip_text = f"🎯 {str(r['OBJETIVO']).strip()} | 👤 {str(r['SUPERVISOR']).strip()}"
                folium.Marker(
                    [r['LATITUD'], r['LONGITUD']], 
                    tooltip=folium.Tooltip(tooltip_text, sticky=True),
                    icon=folium.Icon(color="blue", icon="shield", prefix="fa")
                ).add_to(m)
            st_folium(m, width="100%", height=500, key=f"mapa_{sup_norm}")
        else:
            st.error("No se encontraron objetivos. Verifica en Google Sheets que los nombres de supervisor sean idénticos.")
