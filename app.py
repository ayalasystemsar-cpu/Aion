...
# C. ROL: JEFE DE OPERACIONES
    ...
    # Lógica de Jefe con Mapa
    df_jefe = df_objetivos.dropna(subset=['LATITUD', 'LONGITUD'])
    m_jefe = folium.Map(location=[-34.6, -58.4], zoom_start=12, tiles="CartoDB dark_matter")
    for _, r in df_jefe.iterrows():
        folium.Marker([r['LATITUD'], r['LONGITUD']], tooltip=r['OBJETIVO']).add_to(m_jefe)
    st_folium(m_jefe, width="100%", height=400)
...
# E. ROL: GERENCIA
    ...
    centro = [df_ger['LATITUD'].mean(), df_ger['LONGITUD'].mean()] if not df_ger.empty else [-34.6, -58.4]
    m_ger = folium.Map(location=centro, zoom_start=12, tiles="CartoDB dark_matter")
