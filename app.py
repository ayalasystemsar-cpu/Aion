...
# B. ROL: JEFE DE OPERACIONES
    ...
    # ... (Tu lógica de Jefe de Operaciones)

# C. ROL: SUPERVISOR
elif st.session_state.rol_sel == "SUPERVISOR":
    # ... (Tu lógica de Supervisor)
    ...
    with tab_pres:
        with st.form(key="form_fichaje_vigilador", clear_on_submit=True):
            ...
            if st.form_submit_button("CONSIGNAR PRESENTE"):
                ...
                else: st.warning("⚠️ Faltan datos o captura.")
    ...
    with tab_rev:
        with st.form(key="form_relevo", clear_on_submit=True):
            ...
            if st.form_submit_button("SANCIONAR RELEVO"):
                if v_sal and v_ent:
                    fh = obtener_hora_argentina()
                    escribir_registro_nube("VIGILADORES", [fh.split(" ")[0], fh.split(" ")[1], v_obj_rel, v_sal, v_ent, "N/A", "RELEVO"])
                    st.success("🔄 RELEVO SANCIONADO")
    st.markdown('</div>', unsafe_allow_html=True)

# E. ROL: GERENCIA
elif st.session_state.rol_sel == "GERENCIA":
    # ... (Tu lógica de Gerencia)

# F. ROL: ADMINISTRADOR
elif st.session_state.rol_sel == "ADMINISTRADOR":
    # ... (Tu lógica de Administrador)
