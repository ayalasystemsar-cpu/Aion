...
# D. ROL: SUPERVISOR
    ...
    if not st.session_state.sup_autenticado:
        ...
        with t_p:
            ...
            if not df_p.empty:
                # Filtrar si la hoja de presentismo tiene una columna de 'SUPERVISOR' 
                # (Asegúrate de que el nombre del supervisor coincida con la columna)
                st.dataframe(df_p.sort_values(by=df_p.columns[0], ascending=False), use_container_width=True)
            else:
                st.info("Sin registros de presentismo en el sistema.")
# E. ROL: GERENCIA
