...
import cv2
from pyzbar.pyzbar import decode  # <--- PARA DECODIFICAR QR DE FORMA NATIVA Y PRECISA
...
def aplicar_identidad_alfa():
    st.markdown("""
        ...
        /* --- VISOR TÁCTICO CUADRADO PERFECTO CON GUÍAS DE ESQUINAS BLANCAS --- */
        .visor-qr-tactico {
            position: relative;
            width: 320px;
            height: 320px;
        ...
        /* Las cuatro esquinas guía en formato cuadrado simétrico */
        .visor-qr-tactico::before, .visor-qr-tactico::after {
            content: "";
            position: absolute;
            width: 240px;
            height: 240px;
        ...
        .visor-qr-tactico::before {
            border-top-color: #FFFFFF;
            border-left-color: #FFFFFF;
            top: 35px;
            left: 35px;
        }
        .visor-qr-tactico::after {
    ...
    if st.session_state.sup_autenticado:
        ...
        with t_vis_qr:
            ...
            if not df_objetivos_filtrados.empty:
                ...
                st.markdown("---")
                st.markdown("### 📷 ESCANEO TÁCTICO DE CÓDIGO QR DE PUESTO")
                st.info("Seleccione el movimiento, capture o cargue la foto del código QR de puesto dentro del visor cuadrado simétrico.")
                ...
                # --- VISOR TÁCTICO PERSONALIZADO CON CUADRADO PERFECTO Y ESQUINAS BLANCAS ---
                st.markdown('<div class="visor-qr-tactico">', unsafe_allow_html=True)
                foto_qr_capturada = st.camera_input("Capturar Código QR", label_visibility="collapsed")
                st.markdown('</div>', unsafe_allow_html=True)

                if foto_qr_capturada is not None:
                    try:
                        image = Image.open(foto_qr_capturada)
                        imagen_cv = cv2.cvtColor(np.array(image), cv2.COLOR_RGB2BGR) if 'np' in globals() else image
                        
                        # Decodificación nativa con pyzbar
                        decoded_objects = decode(image)
                        codigo_qr_leido = decoded_objects[0].data.decode('utf-8') if decoded_objects else None
                        ...
                        if codigo_qr_leido:
                            ...
                            st.warning("⚠️ No se detectó ningún código QR válido en la captura. Intente nuevamente enfocando bien dentro del cuadro blanco.")
                    except Exception as e:
                        st.error(f"⚠️ Error al procesar la imagen del QR: {e}")
