...
# C. ROL: SUPERVISOR (FILTRADO PERIMETRAL EXACTO Y MAPAS CON CENTRO DINÁMICO)
    ...
    if not st.session_state.sup_autenticado:
        ...
        # Filtramos df_objetivos basándonos exclusivamente en el supervisor logueado
        ...
        with t_vis_qr:
            ...
            # --- MOTOR DE RENDERIZADO TÁCTICO USANDO df_objetivos_filtrados ---
            if not df_objetivos_filtrados.empty:
                ...
                for _, r in df_objetivos_filtrados.iterrows():
                    html_tooltip = f"""<div style="width: 180px;">🎯 <b>OBJ:</b> {str(r['OBJETIVO']).strip()}<br>👤 <b>SUP:</b> {sup_activo_normalizado}</div>"""
                    folium.Marker(
                        [r['LATITUD'], r['LONGITUD']], 
                ...
                st.warning("⚠️ No se encontraron objetivos para este supervisor.")
            st.markdown('</div>', unsafe_allow_html=True)
