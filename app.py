...
# --- IMPORTACIONES CRÃTICAS DE MAPAS ---
import folium
from folium.plugins import AntPath
from streamlit_folium import st_folium
import math

# ConfiguraciÃ³n de pÃ¡gina OLED
st.set_page_config(
...
# --- 3. FUNCIONES DE LÃ“GICA Y DATOS ---
...
@st.cache_data(ttl=5) # 5 segundos de TTL para actualizaciÃ³n tÃ¡ctica veloz
...
with st.sidebar:
    ...
    st.subheader("ðŸ›¡ï¸ PANEL DE CONTROL")
    
    if st.button("ðŸ›°ï¸ MONITOREO", use_container_width=True):
        st.session_state.rol_sel = "MONITOREO"
        st.session_state.user_sel = "OPERADOR CENTRAL"
        st.session_state.sup_autenticado = False
        st.rerun()
        
    if st.button("ðŸ“‹ JEFE DE OPERACIONES", use_container_width=True):
        st.session_state.rol_sel = "JEFE DE OPERACIONES"
        st.session_state.user_sel = "JEFE DE OPERACIONES"
        st.session_state.sup_autenticado = False
        st.rerun()
        
    if st.button("ðŸ¢ GERENCIA", use_container_width=True):
        st.session_state.rol_sel = "GERENCIA"
        st.session_state.user_sel = "DIRECCIÃ“N GENERAL"
        st.session_state.sup_autenticado = False
        st.rerun()

    with st.expander("ðŸ‘¤ SUPERVISORES", expanded=(st.session_state.rol_sel == "SUPERVISOR" or 'intentando_sup' in st.session_state)):
        ...
        pass_sup = st.text_input("CONTRASEÃ‘A CRÃTICA", type="password", key="auth_pass_sup")
        ...
        if st.button("AUTENTICAR E INGRESAR", use_container_width=True):
            ...
            if user_sup.strip().lower() == usuario_esperado and pass_sup == "1234":
                ...
                st.success(f"ðŸ”“ ACCESO CONCEDIDO: {nom_sup}")
                st.rerun()
            else:
                st.session_state.sup_autenticado = False
                st.error("âŒ CREDENCIALES INVÃLIDAS EN BASE")

    st.write("---")
    if st.button("ðŸ‘® VIGILADOR (ACCESO PUESTO)", use_container_width=True):
    ...
    st.write("---")
    st.markdown("**âš™ï¸ ADMINISTRADOR**")
    if st.button("ACCEDER AL NÃšCLEO MAESTRO", use_container_width=True):
    ...
    if st.button("ACTIVAR\nPÃNICO", type="primary"):
        ...
        escribir_registro_nube("ALERTAS", [obtener_hora_argentina(), st.session_state.user_sel, "PÃNICO", "PENDIENTE", carga_sos])
        st.error(f"ðŸš¨ S.O.S ENVIADO DESDE {obj_alerta}")

# --- 6. CABECERA CENTRAL ---
st.markdown('<div class="contenedor-logo-central"><img src="https://raw.githubusercontent.com/ayalasystemsar-cpu/Aion/main/assets/LOGO%20-%20AION-YAROKU.jpeg" class="logo-phoenix"></div>', unsafe_allow_html=True)

titulos = {
    "MONITOREO": "ðŸ›°ï¸ CENTRAL DE INTELIGENCIA OPERATIVA",
    "SUPERVISOR": f"ðŸ“± EstaciÃ³n de Control: {st.session_state.user_sel}",
    "VIGILADOR": "ðŸ‘® TERMINAL OPERATIVO VIGILADORES",
    "JEFE DE OPERACIONES": "ðŸ“‹ COMANDO DE OPERACIONES TÃCTICAS",
    "GERENCIA": "ðŸ¢ DIRECCIÃ“N Y FISCALIZACIÃ“N GENERAL",
    "ADMINISTRADOR": "âš™ï¸ NÃšCLEO MAESTRO: AION-YAROKU"
}
st.markdown(f'<div class="estacion-titulo">{titulos.get(st.session_state.rol_sel, "SISTEMA TÃCTICO DE COMANDO")}</div>', unsafe_allow_html=True)
...
if st.session_state.rol_sel == "MONITOREO":
    ...
    c1.metric("ðŸš¨ S.O.S ACTIVOS", sos_activos)
    c2.metric("ðŸ“¡ RED", "OPERATIVA")
    c3.metric("ðŸ•’ HORA LOCAL", obtener_hora_argentina().split(" ")[1])

    t_radar, t_gestion, t_comunicacion, t_pres, t_vig, t_guardia = st.tabs([
        "ðŸš¨ RADAR S.O.S", "ðŸ“– LIBRO DE BASE", "ðŸ’¬ CHAT OPERATIVO", "ðŸ“‹ PRESENTISMO GENERAL", "ðŸ‘¥ PADRÃ“N VIGILADORES", "ðŸ”„ NOVEDADES GUARDIA"
    ])
    
    with t_radar:
        st.subheader("ðŸ“¡ RADAR GLOBAL DE OBJETIVOS")
        if sos_activos > 0:
            ...
            with st.form(key="form_finalizar_panico", clear_on_submit=True):
                ...
                txt_informe_cierre = st.text_area("INFORME OPERATIVO DE CIERRE:", placeholder="Describa la resoluciÃ³n...")
                if st.form_submit_button("ðŸš¨ FINALIZAR PÃNICO Y NORMALIZAR") and txt_informe_cierre.strip():
        ...
        if not df_mapa_monitoreo.empty:
            ...
            for _, r in df_mapa_monitoreo.iterrows():
                info_hover = f"ðŸŽ¯ OBJETIVO: {r['OBJETIVO']} | ðŸ‘¤ SUPERVISOR: {r.get('SUPERVISOR', 'NO ASIGNADO')}"
                folium.CircleMarker(
    ...
    with t_gestion:
        st.subheader("ðŸ“– HISTORIAL DE OPERATIVOS")
    ...
    with t_comunicacion:
        with st.form(key="form_chat_monitoreo", clear_on_submit=True):
            txt_mensaje_mon = st.text_input("ESCRIBIR MENSAJE TÃCTICO GENERAL:")
    ...
    with t_pres:
        st.subheader("ðŸ“‹ TABLA MASTER: PRESENTISMO")
    ...
    with t_vig:
        st.subheader("ðŸ‘¥ TABLA MASTER: RELEVOS VIGILADORES")
        ...
        if df_padrero is not None and not df_padrero.empty:
            ...
            st.info("No hay datos en la pestaÃ±a de relevos (Vigiladores).")

    with t_guardia:
        st.subheader("ðŸ”„ TABLA MASTER: NOVEDADES_GUARDIA")
...
# B. ROL: SUPERVISOR (SISTEMA DE FILTRADO INTELIGENTE BLINDADO)
    ...
    if st.session_state.sup_autenticado:
        ...
        st.subheader("Control de Unidad MÃ³vil")
        st.markdown('<div class="panel-info">', unsafe_allow_html=True)
        c1, c2, c3, c4 = st.columns(4)
        with c1: st.selectbox("MÃ³vil:", ["S-001", "M-002", "M-003", "OTRO"], key="sup_movil_select")
        ...
        with col_btn1: st.button("SELLAR ODOMETRÃA Y LOGÃSTICA", key="btn_sellar_logistica", use_container_width=True)
        with col_btn2:
            if st.button("ðŸ”„ REFRESCAR SISTEMA", key=f"btn_refrescar_sistema_{sup_activo_normalizado}", use_container_width=True): st.rerun()

        t_vis_qr, t_car_tac, t_com_sup, t_pres_sup = st.tabs(["Visita QR", "Carga TÃ¡ctica", "ðŸ’¬ CHAT OPERATIVO", "ðŸ“‹ NOVEDADES Y RELEVOS"])
        ...
        with t_vis_qr:
            ...
            st.radio("ACCIÃ“N:", ["SELECCIONAR...", "INGRESO", "SALIDA"], index=0, key="sup_radio_accion", horizontal=True)
            ...
            if not df_mapa_sup.empty:
                ...
                for _, r in df_mapa_sup.iterrows():
                    folium.Marker([r['LATITUD'], r['LONGITUD']], tooltip=f"ðŸŽ¯ OBJETIVO: {r['OBJETIVO']}", icon=folium.Icon(color="blue", icon="shield", prefix="fa")).add_to(m_visor)
        ...
        with t_com_sup:
            with st.form(key="form_chat_supervisor", clear_on_submit=True):
                txt_mensaje_sup = st.text_input("REPORTE RÃPIDO PARA MONITOREO:")
        ...
        with t_pres_sup:
            st.markdown("### ðŸ“‹ NOVEDADES DE MI GRUPO ASIGNADO")
...
# --- C. ROL: VIGILADOR ---
    ...
    tab_presentismo, tab_relevo = st.tabs([
        "ðŸ“‹ FICHAJE INDIVIDUAL (PRESENTISMO)", 
        "ðŸ”„ SANCIONAR RELEVO (CAMBIO DE GUARDIA)"
    ])
    
    with tab_presentismo:
        st.markdown("### ðŸ“¸ REGISTRO BIOMÃ‰TRICO DE INGRESO")
        with st.form(key="form_fichaje_vigilador", clear_on_submit=True):
            ...
            v_tipo_marcacion = st.selectbox("TIPO DE MARCACIÃ“N:", ["INGRESO", "EGRESO"], key="tipo_marc_vig")
            ...
            if btn_fichar:
                if v_apellido and img_facial and v_dni:
                    ...
                    # SeparaciÃ³n lÃ­mpida obligatoria de Fecha y Hora
                    fecha_hora_arg = obtener_hora_argentina()
                    fecha_hoy = fecha_hora_arg.split(" ")[0]
                    hora_hoy = fecha_hora_arg.split(" ")[1]
                    
                    # Estructura EXACTA segÃºn Captura 1 (PestaÃ±a PRESENTISMO):
                    ...
                    # Espejo de auditorÃ­a unificado y alineado en NOVEDADES_GUARDIA:
                    ...
                    if exito_pres:
                        st.success(f"ðŸ”’ BIOMETRÃA REGISTRADA: MarcaciÃ³n de {v_tipo_marcacion} guardada correctamente.")
                    else:
                        st.error("âŒ ERROR DE RED: No se pudo impactar en la planilla de Presentismo.")
                else:
                    st.error("âŒ ERROR: Complete todos los campos y la captura facial.")
                    
    with tab_relevo:
        st.markdown("### ðŸ”„ REGISTRO FORMAL DE CAMBIO DE GUARDIA")
        with st.form(key="form_relevo_vigilador_directo", clear_on_submit=True):
            ...
            if btn_relevo:
                if vig_saliente and vig_entrante:
                    ...
                    # Estructura EXACTA segÃºn Captura 2 (PestaÃ±a VIGILADORES):
                    ...
                    if exito_relevo:
                        st.success(f"âš¡ RELEVO SANCIONADO: Registro guardado en la pestaÃ±a Vigiladores.")
                    else:
                        st.error("âŒ ERROR DE RED: No se pudo impactar el relevo.")
                else:
                    st.error("âŒ ERROR: Complete los campos de personal saliente y entrante.")
    st.markdown('</div>', unsafe_allow_html=True)

# --- D. JEFE DE OPERACIONES ---
elif st.session_state.rol_sel == "JEFE DE OPERACIONES":
    col1, col2, col3, col4 = st.columns(4)
    col1.metric("ðŸš¨ S.O.S ACTIVOS", "0")
    col2.metric("ðŸ“¡ RED", "OPERATIVA")
    col3.metric("ðŸ‘¤ USUARIO", f"{st.session_state.user_sel}")
    col4.metric("ðŸ•’ HORA LOCAL", obtener_hora_argentina().split(" ")[1])

    t_crisis, t_ejecucion, t_auditoria = st.tabs(["Centro de Crisis", "EjecuciÃ³n", "AuditorÃ­a"])
    
    with t_crisis:
        st.subheader("ðŸ“¡ RADAR Y LOCALIZACIÃ“N DE OBJETIVOS")
        st.markdown('<div class="radar-box">', unsafe_allow_html=True)
    ...
    with t_ejecucion:
        st.markdown('<div class="panel-novedad">', unsafe_allow_html=True)
        st.subheader("ðŸš¨ PETICIÃ“N DE ALTA/BAJA")
        o_accion = st.selectbox("AcciÃ³n:", ["ALTA", "BAJA"])
        o_cat = st.selectbox("CategorÃ­a:", ["OBJETIVO", "MÃ“VIL", "RECURSO HUMANO"])
        o_det = st.text_input("Nombre / Detalle:")
        if st.button("ELEV AR PETICIÃ“N"):
            if o_det.strip():
                escribir_registro_nube("PETICIONES", [obtener_hora_argentina(), st.session_state.user_sel, o_accion, o_cat, o_det])
                st.success("âœ… PeticiÃ³n Elevada Exitosamente")
        st.markdown('</div>', unsafe_allow_html=True)

    with t_auditoria:
        st.subheader("ðŸ“‹ REPORTE DE MOVIMIENTOS")
...
# --- E. GERENCIA ---
    ...
    st.markdown('<h2 style="color:#00E5FF; font-family:\'Orbitron\', sans-serif; font-size:24px; margin-bottom:5px;">Comando EstratÃ©gico: DIRECCIÃ“N GENERAL</h2>', unsafe_allow_html=True)
    m1, m2, m3, m4 = st.columns(4)
    m1.metric("Ahorro de Riesgo (Estimado)", "$ 1.200.000")
    m2.metric("Nivel de Cobertura", "47/93")
    m3.metric("AuditorÃ­as FÃ­sicas (QRs)", "2")
    m4.metric("Desgaste Flota (Km)", "4954 Km")
    
    t_com_est, t_ejecucion_ger, t_tab_auditoria = st.tabs(["ðŸ“© COMUNICACIÃ“N ESTRATÃ‰GICA", "ðŸŽ® EJECUCIÃ“N", "ðŸ“ TABLERO DE AUDITORÃA"])
    ...
    with t_com_est:
        ...
        if st.button("Ejecutar Directiva"):
            ...
            st.success("âœ… Directiva Transmitida")
        st.markdown('</div>', unsafe_allow_html=True)
    ...
    with t_ejecucion_ger:
        ...
        with col_g1:
            ...
            if st.button("Solicitar Alta"):
                ...
                st.success("âœ… PeticiÃ³n enviada")
            st.markdown('</div>', unsafe_allow_html=True)
        with col_g2:
            st.markdown('<div class="panel-novedad">', unsafe_allow_html=True)
...
# H. ROL: ADMINISTRADOR
    ...
    if u_ing == "admin" and p_ing == "aion2026": st.success("NÃºcleo Maestro desbloqueado.")
