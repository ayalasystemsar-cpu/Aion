...
# A. ROL: MONITOREO
    ...
    # Aquí están las pestañas con PRESENTISMO al final
    ...
    with t_radar:
        ...
        if sos_activos > 0 and comisaria_cercana:
            try:
                folium.Marker([comisaria_cercana['LAT'], comisaria_cercana['LON']], tooltip=f"🚓 {comisaria_cercana['NOMBRE']}", icon=folium.Icon(color="blue", icon="shield-halved", prefix="fa")).add_to(m_mon)
                AntPath(locations=[[comisaria_cercana['LAT'], comisaria_cercana['LON']], [lat_foco, lon_foco]], color='#FFEB3B', weight=6, delay=600).add_to(m_mon)
            except: pass

        st_folium(m_mon, width="100%", height=450, key="mapa_final_monitoreo_v30")
    ...
    with t_pres:
        ...
        else: st.info("No hay registros de presentismo aún.")
...
# C. ROL: JEFE DE OPERACIONES
    ...
    st.write("Panel Jefe de Operaciones")
...
# D. ROL: SUPERVISOR
    ...
    with t_vis_qr:
        st.write("Gestión de Visitas")

# E. ROL: GERENCIA
elif st.session_state.rol_sel == "GERENCIA":
    st.write("Panel Gerencia")
